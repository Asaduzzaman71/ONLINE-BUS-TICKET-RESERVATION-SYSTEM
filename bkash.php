...
<body>
    ...
    <div class="container-fluid bg">
		...
		$link = mysqli_connect("localhost", "root", "", "demo");
			...
			$sql = "SELECT ticket_id from registration_table ORDER BY ticket_id desc LIMIT 1";
   ...
   <?php
       ...
       if(isset($_POST['gett'])){
            ...
            $num=$_POST['numc'];
            $sql = " SELECT * FROM users  WHERE  mobile='$num' ";
            ...
            $id=$_POST['idt'];
            $number=$_POST['numc'];
            $sql = "SELECT * FROM registration_table where ticket_id='$id' AND mbn='$num'";
            ...
            if($result = mysqli_query($link, $sql)){
                if(mysqli_num_rows($result) > 0){
                    ...
                    {           
                        ...
                        $Origin=$row['oc'] ;
                        $Destination= $row['dc'] ;
                        $numberofticket= $row['noftckt'];
                        echo 'Origin City:' .$Origin."<br>"; 
                        echo 'Destination City:'.$Destination."<br>";
                        echo 'Number of ticket:'.$numberofticket."<br>";

                        $sql = "SELECT * FROM bus_time where dep='$Origin' AND arr='$Destination' LIMIT 1";
                        if($result = mysqli_query($link, $sql)){
                         if(mysqli_num_rows($result) > 0){
                            while($row = mysqli_fetch_array($result)){
                                  $price=$row['fare'] ;
                        ...
                        $total1 = ($numberofticket * $price);
                        $total2 = (10 * $total1) / 100;
                         $total3=$total1-$total2;
                         echo 'Grand total'.$total3;
